...
{
    if (missing(ask)) {
      ask <- if (is.R()) { #we are using R
        dev.interactive() #is the current graphics device interactive?
      }
      else {
        interactive() #Is R used interactivly?
      }
    }
    oldpar <- NULL
    on.exit(par(oldpar)) # query graphical parameters. 
    if (auto.layout) 
...
function (x) 
{
  ...
  #cov2cor scales a covariance matrix into the corresponding correlation matrix efficiently.
    cor(as.matrix(x)) # "cor" from package stats

"crosscorr.plot" <- # do we need this??
...
"pretty.discrete" <- function(y, right)
{
    ...
    yunique <- unique(y) #  returns a vector, data frame or array like x but with duplicate elements/rows removed.
    if (length(yunique) == 1) {
        return(ybreaks)
    }
...
"densplot" <- ## skip?? seems to be related to plotting
          ...
          right=TRUE, ...) 
{
    ...
    for (i in 1:nvar(x)) {
        y <- xx[, i, drop = TRUE]
        if (missing(bwf))  ## was the parameter supplied in this function
            bwf <- function(x) {
                x <- x[!is.na(as.vector(x))]
                
# IQR {stats} function computes the quartiles using the quantile function rather than following Tukey's recommendations, i.e., IQR(x) = quantile(x, 3/4) - quantile(x, 1/4).
...
## diff are subsequent differences
...
"traceplot" <- #prolly drawing
function (x, smooth = FALSE, col = 1:6, type = "l", xlab = "Iterations",
          ylab = "", ...) 
{
    x <- mcmc.list(x) # in this here package
    args <- list(...)
    for (j in 1:nvar(x)) {
        xp <- as.vector(time(x))
        yp <- if (nvar(x) > 1) 
        ...
        if (smooth) {
            ...
            # lowess (stats),Scatter Plot Smoothing
  ...
  function (object, quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975), ...) 
{
  ...
  # base::drop, Delete the dimensions of an array which have only one level.
