...
{
  x <- as.mcmc.list(x)
  # nchain is defined in mcextratot.R
  if (nchain(x) < 2) 
    stop("You need at least two chains")
  ## RGA added an autoburnin parameter here, because if I have already
  ## trimmed burn in, I don't want to do it again.
  ## "start", "end" (stats) encode terminal times of a time series
  if (autoburnin && start(x) < end(x)/2 ) 
  ## extract the data between start and end from timeseries
    x <- window(x, start = end(x)/2 + 1)  ##mokey
  Niter <- niter(x) # niter is in this package
  Nchain <- nchain(x) #nchain pev
  Nvar <- nvar(x) #nvar is in this package
  xnames <- varnames(x) #varnames is in this package
  
  if(transform)
    x <- gelman.transform(x)  # in this package
  ##
  ...
  if(Nvar > 1 && multivariate) {
      ...
      if (is.R()) {
          ...
          # eigen(base) Give spectral decomposition of matrix
  ...
  # diag (base)  Extract or replace the diagonal of a matrix, or construct a
  #   diagonal matrix
  w <- diag(W)
  b <- diag(B)
